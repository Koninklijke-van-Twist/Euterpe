...
describe("buildMpvArgs", () => {
  it("uses alsa audio output on linux by default", () => {
    ...
    assert.ok(args.includes("--pause=no"));
  ...
  it("appends audio device and extra mpv args", () => {
    const args = buildMpvArgs({
      ipc: "/tmp/mpv.sock",
      isWindows: true,
      mpvAo: null,
      mpvAudioDevice: "alsa/default",
      mpvExtraArgs: ["--volume-max=100"],
    });
    assert.ok(args.includes("--process-instance=multi"));
    assert.ok(args.includes("--audio-device=alsa/default"));
    assert.ok(args.includes("--volume-max=100"));
  });
});
